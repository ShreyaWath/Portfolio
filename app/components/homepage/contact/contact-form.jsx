...
function ContactForm() {
  const [error, setError] = useState({ email: false, required: false });
  const [isLoading, setIsLoading] = useState(false);
  const [userInput, setUserInput] = useState(null); // Start with null to prevent SSR mismatch
  const [isClient, setIsClient] = useState(false); // Ensure client-side rendering

  // Initialize state on client to prevent hydration issues
  useEffect(() => {
    setUserInput({ name: "", email: "", message: "" });
    setIsClient(true);
  }, []);

  if (!isClient || !userInput) return null; // Prevent rendering during SSR
      ...
      <div className="max-w-3xl text-white rounded-lg border border-[#464c6a] p-3 lg:p-5">
        ...
        <div className="mt-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <button
              className="flex items-center gap-1 hover:gap-3 rounded-full bg-gradient-to-r from-pink-500 to-violet-600 px-5 md:px-12 py-2.5 md:py-3 text-xs md:text-sm font-medium uppercase tracking-wider text-white transition-all duration-200 ease-out"
              onClick={handleSendMail}
              disabled={isLoading}
            >
              {isLoading ? "Sending Message..." : "Send Message"}
              {!isLoading && <TbMailForward size={20} />}
            </button>
          </div>
        </div>
      </div>
